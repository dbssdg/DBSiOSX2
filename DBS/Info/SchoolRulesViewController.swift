...
//        })
    ...
    schoolRulesTextView.scrollRangeToVisible(NSRange(location: 0, length: 0))
        schoolRulesTextView.text = {"""
        a. Boys who want to obtain an early-leave from the School due to sickness must report to the School Nurse. A sick-leave acknowledgement will be issued. With such acknowledgement, the boy can obtain the Early Leave slip from the General Office. School Nurse may decline the request for early-leave of boys.
        b. Boys who want to obtain an early-leave from the School due to personal or family affairs are required to inform the Class Teacher and General Office with an early leave form with parents’ signature. A leave-slip will be issued by the Dean. Those who do not have the signed early leave form from the parents are required to obtain written permission from the Headmaster.
        c. The security guard at the main entrance has the right to ask the early-leave boys to show the early leave slip issued by the General Office / jot down their names and student numbers in a log-book. Boys leaving the School without written permission from the School Authorities are considered to be playing truant.
        d. On returning to the School, after absence of any length of time, boys MUST bring an absence notification form with parents’ signature and/or a doctor certificate, giving the reason for the absence and the dates on which they have been away from School. Boys must hand in the documents to the Class Teacher on the first day of returning to School so that the Class Teacher will submit the form and supporting documents to General Office for attendance record amendment.
        e. Any boy who is absent for more than two days should get his parents to notify the Headmaster through his Class Teacher.
        f.  Swimming Gala, Sports’ Day and Speech Day are special function days. To support the School, boys are required to be present. Absentees that can produce doctor’s certificate may be exempted.
        """}()
    }
